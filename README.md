...
- **Docker path:** make sure `docker compose up postgres` finished its healthcheck before running migrations.
- **Native path:** confirm Postgres is running (`pg_isready`) and that `DB_HOST=localhost` is set in `.env`.

### `extension "vector" does not exist`

```bash
# Connect as a superuser and install it
...
### Secret file not found / empty credentials

Check that the file exists and is not empty:

```bash
cat infra/secrets/db_password.txt
```

The `read_secret()` function in `config.py` will silently return `""` if the file is missing, which causes an auth failure at connection time.
...
If `sentence-transformers` is not importable, `EMBEDDINGS_ENABLED` is set to `False` and embeddings are silently skipped — items are still ingested without vectors.
